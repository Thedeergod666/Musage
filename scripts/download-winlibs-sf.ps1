# 从 SourceForge 直链下载 WinLibs（与 GitHub 版是同一份二进制）
# SourceForge 直链会 302 跳到 mirror
$ErrorActionPreference = "Stop"
$url = "https://sourceforge.net/projects/winlibs-mingw/files/v16.1.0posix-14.0.0-ucrt-r2/winlibs-x86_64-posix-seh-gcc-16.1.0-mingw-w64ucrt-14.0.0-r2.zip/download"
$dst = "C:\tools\winlibs.zip"

if (-not (Test-Path "C:\tools")) { New-Item -ItemType Directory -Path "C:\tools" | Out-Null }
if (Test-Path $dst) { Remove-Item $dst -Force }

Write-Host "[1/3] downloading from SourceForge..."
Write-Host "      url: $url"
Write-Host "      dst: $dst"

# WebClient (跟随 SF 302 跳转)
$wc = New-Object System.Net.WebClient
$wc.Headers.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36")
...
Write-Host "[2/3] downloaded: $size MB"

if ($size -lt 100) {
    Write-Host "[warn] 文件太小（$size MB），可能是 SF 错误页。请检查后重试。"
    exit 1
}
