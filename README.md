...
| 密钥存储 | OS keyring（Windows Credential Manager）|
...
**旧 schema（已从 ccswitch 源码确认 6/1 前）**：
...
```json
{
  ...
  "model_remains": [{
    ...
    "current_interval_usage_count":  56,    // 注意：字段名"usage"实为"剩余"
    "end_time": 1748500000000,
...
**6/1 后 schema 未知**：`api.rs::parse_tier` 用多组字段名候选 + 失败时打印原始 JSON。
...
| 拉不到数据 / `未返回结果` | MiniMax 改了 schema | `cargo run -- dump` 看新字段，更新 `api.rs::parse_tier` |
