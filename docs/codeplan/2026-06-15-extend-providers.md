...
> 日期: 2026-06-15（创建） / 2026-06-16（PR 2 完成更新 / PR 3 完成更新）
> 状态: **PR 2 ✅ + PR 3 ✅ 完成。Phase 6 / Phase X 仍待执行。**
...
| **PR 3** = Phase 3+4+5 | CustomSource + IPC + 设置面板重构 | ✅ **完成**（2026-06-16，3 commit） | `68b5dff` (骨架) + `b234e45` (IPC) + `bb5151c` (前端) |
...
### PR 3 实际交付（2026-06-16）

按 plan §1 目标，Musage 现有 **13 个内置 quota source + N 个用户自定义 New API**。

**后端骨架**（commit `68b5dff`）：
- `QuotaSource::id/display_name` 改 `Cow<'_, str>`，13 内置 impl 改 `Cow::Borrowed(...)`（零分配）
- 新增 `all_sources(state) async` 包装层（builtin + customs 拼接）；`find_source` 改 async
- 新增 `providers/parse.rs`：`read_path("a.b[0]")` + `num_f64` 兼容字符串数字（**14 单测**）
- 新增 `providers/custom.rs`：`CustomSourceSpec` + `ExtractSpec` enum（3 预设）+ `CustomSource::QuotaSource` impl（**14 单测**）
- 新增 `config/custom_sources.rs`：`load/save_custom_sources` + 原子写 + parse 失败 backup 到 `.bak.<ts>`（**6 单测**）
- `AppState` 加 `custom_sources: Arc<RwLock<Vec<CustomSourceSpec>>>`，启动时 load

**5 个 IPC command**（commit `b234e45`）：
- `commands.rs` 拆出 `commands/mod.rs` + `commands/custom_sources.rs`（1200+ 行太重）
- `list_custom_sources` / `add_custom_source`（生成 `custom_<uuid>` + 立即 refresh_single）/
  `update_custom_source` / `delete_custom_source`（清 keys.json） / `test_custom_source`（**2 单测**）
- 锁顺序约定文档化：`config.lock → custom_sources.lock`，**绝不**反过来

**前端 UI**（commit `bb5151c`）：
- `types.ts`：`ProviderId` 改 `string`（dynamic id）；加 `ExtractSpec` / `CustomSourceSpec` interface
- `main.ts`：`ProviderSnapshot.provider` 改 `string`；加 `accent?` / `display_name?` 透传（CustomSource 后端透传）
- `api.ts`：5 个 typed wrappers
- `groups.ts`（新）：6 组分组（`token_plan/balance/official/xiaomi/custom/misc`） + 原生 `<details>/<summary>`
- `modal.ts`（新）：原生 `<dialog>` 包装（a11y + ESC + backdrop 全免费，0 依赖）
- `custom-source-form.ts`（新）：「+ 添加」表单 + 3 选 1 extract preset（new_api/balance/custom）+ 8 色 accent 调色板 + 「测试连接」按钮
- `providers.ts`：重构 toolbar / 分组 / 搜索布局；custom source 面板加 🗑️ 二次输入 display_name 删除
- `settings.css`：`.provider-toolbar` / `.provider-group` / `dialog.modal` / `.accent-swatch` / `.btn-delete-custom`
- `AGENTS.md`：同步 provider 列表 + 文件结构 + PR 3 设计决策

**验证**：
- `cargo check` 0 错（15 个 pre-existing warning，0 新警告）
- `cargo test providers::parse / providers::custom / config::custom_sources / commands::custom_sources`：**36 passed; 0 failed**
- `pnpm tsc --noEmit`：0 错
- `pnpm vite build`：通过（settings.js 49.67 → 59.19 kB，+9.5 kB for groups/modal/form UI）
...
5. **Trait 改 `Cow<'_, str>` 决策**（PR 3 review）：原本担心传染 26+ 调用方，实际只影响 7 处（`s.id()` → `s.id().as_ref()`），多数是 .id() 跟 &str 比较（自动 deref）。前端**完全无传染**（IPC 边界 String 化）。
6. **`builtin_sources()` 保持 sync**（PR 3 review）：`setup` 闭包不能 await，所以不全局改 async；加 `all_sources(state)` async 包装层做拼接。dump CLI 暂不接 customs（YAGNI）。
7. **`add_custom_source` 立即 refresh_single**（PR 3 review 用户决策）：不等 poller，浮窗 0 延迟看到新卡片 + 数据。代价 1 次额外 HTTP，用户感知"流畅"。
8. **Extract 模板 v1 = 3 选 1**（PR 3 review 用户决策）：New API 系 / 余额系 / 自定义 path。比"2 预设"多覆盖异构 schema，比"完整表达式引擎"少一个工作量档。
9. **`PROVIDER_META` 表本体不动**（PR 3 review）：浮窗 `updateCard` 走 `PROVIDER_META[id] ?? { name: snap.display_name ?? id, accent: snap.accent ?? "#888" }`，CustomSource 透传 display_name/accent 走 snapshot 字段。
...
完成后 Musage 总共支持 **13 个内置 + N 个用户自定义 New API** quota source：
...
- **N 个用户自定义**：custom_<uuid>（New API 通用，用户在设置面板里加/改/删；上限 50）
