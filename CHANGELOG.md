...
- **Kimi/Code 占比拆分以文本行呈现**：总套餐行保持普通单条**阈值变色** bar（pct 大字和 bar 颜色都随总用量 70%/90% 阈值，跟其它行完全一致）；bar 和「月重置」倒计时之间插一行小字 `Kimi xx% · Code xx%`（`.split-note`，样式与 `row-foot` 月重置行完全同步：10.5px `--text-faint` + tabular-nums + text-shadow）。**Kimi = 总用量 − Code**：探测结论 API 无独立 Kimi Work 分项（`GetUsages` 只有 `totalQuota` + `kimiCodeUsedRatio` 两个比值，官方黑段同样是"其余全部"），拆分只能做到「Code vs 其他」——与官网一致。（曾实装官网同款双色堆叠条——黑段 `#17171c` + 蓝段 `#2f7cf6` + legend——黑段在深色玻璃背景上易读性太差，用户拍板砍掉分段改文本行。）
