...
| **StepFun** | Oasis-Token (Cookie) | Step Plan / Credit 套餐 | token 从 platform.stepfun.com DevTools 复制；Oasis-Webid 自动从 token 提取 |
...
| 升级 | 手动：GitHub releases 下载覆盖装（v0.2.0 起移除 tauri-plugin-updater，原因见 [RELEASING.md](RELEASING.md)） |
...
| macOS 跑起来 UI 全是 `provider.minimax.name` / `settings.nav.providers` 这种 raw key | v0.2.1 的 frontend i18n bundle bug（Vite 动态 import 模板字符串没生成 chunk，dicts 永远是空）。**v0.2.2 已修** | 升 v0.2.2+（当前最新 v0.2.4）即可；如果还看到 raw key 说明 dist 没重建，跑 `pnpm build && pnpm tauri build` |
